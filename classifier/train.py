...
class Trainer:
    ...
    def evaluate (self):
        ...
        self.valid_top5acc.reset_states()
        ...
        for i in tqdm(range(num_iterations), ascii=True):
            ...
            self.valid_acc(tf.argmax(labels, 1), tf.argmax(predicts, 1))
            self.valid_top5acc(labels, predicts)
        accuracy = self.valid_acc.result().numpy()
        top5accuracy = self.valid_top5acc.result().numpy()
        acc = self.comm.allreduce(accuracy, op = MPI.MAX)
        acc = accuracy
        self.valid_acc.reset_states()
        self.valid_top5acc.reset_states()
        return acc, top5accuracy
